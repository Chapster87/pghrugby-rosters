...
This document describes the general approach to organizing Next.js page development in the Hydrow site, with context for AIs and developers.
...
- `_content`: Contentful or CMS-specific logic/content mapping
- `_static`: Static configuration or constants for business logic
- `_helpers`: Utilities for stitching data into view models

## Example: Upgrades Page

```
src/app/(pages)/[locale]/[theme]/upgrades/[handle]/
  ├── page.tsx
  ├── _components/
  ├── _data/
  ├── _content/
...
## Example: Rowers Product Page

```
src/app/(pages)/[locale]/[theme]/products/rowers/
  ├── page.tsx
  ├── _components/
  ├── _data/
  ├── _content/
...
- **Integrate data sources via helpers:** Use `_helpers` to combine Shopify, Contentful, and config data into view models.
...
This approach supports scalable, maintainable Next.js development and enables AIs to generate code that fits the Hydrow site's architecture.
